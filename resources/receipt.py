...
from db import db
from schemas.receipt import receipt_schema, receipts_schema
...
class ReceiptResource(Resource):
    ...
    def post(self, mispar_osek):
        if Provider.query.filter_by(mispar_osek=mispar_osek).count():
            return {
                "message": "Provider with mispar-osek={mispar_osek} already exists"
            }, 400
    ...
    def delete(self, mispar_osek):
        try:
            ...
            return {
                "message": f"Provider with mispar-osek={mispar_osek} does not exist"
            }, 400


class ReceiptListResource(Resource):
    def get(self):
        return {"receipts": providers_schema.dump(Provider.query.all())}
