...
def test_get_provider(client, _provider1_in_db):
    resp = client.get(f"{ProviderResource.BASE_ROUTE}/{_provider1_in_db.mispar_osek}")
    assert provider_schema.dump(_provider1_in_db) == resp.json["provider"]


def test_get_non_existing_provider(client, _provider1_obj):
    resp = client.get(f"{ProviderResource.BASE_ROUTE}/{_provider1_obj.mispar_osek}")
    assert resp.is_json
...
def test_create_new_provider(client, _provider1_obj):
    ...
    resp = client.post(
        ...
        json=dict(service_type=_provider1_obj.service_type, name=_provider1_obj.name),
    )
    assert resp.is_json
    assert str(_provider1_obj.mispar_osek) == resp.json["provider"]["mispar_osek"]
...
def test_create_new_provider_with_duplicate_mispar_osek(client, _provider1_obj):
    ...
    assert resp.is_json
    assert resp2.json == {
        "message": f"Provider with mispar-osek={_provider1_obj.mispar_osek} already exists"
    }
...
def test_delete_non_existing_provider(client, _provider1_obj):
    ...
    assert resp_delete.json == {
        "message": f"Provider with mispar-osek={_provider1_obj.mispar_osek} does not exist"
...
def test_update_existing_provider(client, _provider1_in_db):
    ...
    resp_update = client.put(
        f"{ProviderResource.BASE_ROUTE}/{_provider1_in_db.mispar_osek}",
        json=dict(service_type=new_service_type, name=new_name),
    )
